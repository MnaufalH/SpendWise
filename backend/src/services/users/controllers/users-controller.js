...
const createUser = async (req, res, next) => {
    ...
    WalletsRepositories.addWallets(user)

    return response(res, 201, 'Berhasil melakukan Registrasi', { id: user })
}

const getAllUsers = (req, res, next) => {
    const users = UsersRepositories.getAllUsers()
    if (users.length < 1) {
...
const deleteUser = (req, res, next) => {
    ...
    UsersRepositories.deleteUser(user_id)
...
const getUserById = (req, res, next) => {
    ...
    const user = UsersRepositories.getUserById(user_id)
    if (!user) {
