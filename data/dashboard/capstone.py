...
# ════════════════════════════════════════════════════════════════════════════
    ...
    #     <div class='section-header'>
    #         <span class='icon'>🏦</span>
    #         <span class='title'>Pola Loan Repayment Antar Cluster</span>
    #     </div>
    #     <div class='section-desc'>Perbandingan distribusi cicilan di setiap kelompok finansial.</div>
    #     """, unsafe_allow_html=True)

    #     fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 5))
    #     for ax in [ax1, ax2]:
    #         _, ax = setup_ax(fig, ax)

    #     # violin plot
    #     data_by_status = [
    #         df[df["Status_Label"] == s]["Loan_Repayment_Ratio"].dropna() * 100
    #         for s in ["Low", "Medium", "High"]
    #     ]
    #     parts = ax1.violinplot(data_by_status, positions=[0,1,2],
    #                            showmedians=True, showextrema=False)
    #     for i, (pc, color) in enumerate(zip(parts['bodies'], COLOR_MAP.values())):
    ...
    #     ax1.set_xticklabels(["Low","Medium","High"], color="#9aa4b2")
    #     ax1.set_ylabel("Loan Repayment Ratio (%)")
    #     ax1.set_title("Distribusi Cicilan", fontsize=12, fontweight="bold")

    #     # bar rata-rata
    #     loan_mean = df.groupby("Status_Label")["Loan_Repayment_Ratio"].mean() * 100
    #     bars = ax2.bar(
    #         loan_mean.index, loan_mean.values,
    #         color=[COLOR_MAP[s] for s in loan_mean.index],
    #         width=0.5, edgecolor="none"
    #     )
    #     for bar in bars:
    #         h = bar.get_height()
    #         ax2.text(bar.get_x() + bar.get_width()/2, h + 0.3,
    #                  f"{h:.1f}%", ha='center', color='white',
    #                  fontsize=11, fontweight='bold')
    #     ax2.set_ylabel("Rata-rata (%)")
    #     ax2.set_title("Rata-rata Cicilan", fontsize=12, fontweight="bold")
    ...
    #     st.markdown("""
    #     <div class='insight-box'>
    #         Cluster <strong>Low</strong> umumnya menanggung beban cicilan yang lebih bervariasi dan lebih besar,
    #         yang berkontribusi pada ketidakstabilan finansial. Cluster <strong>High</strong> menunjukkan
    #         distribusi cicilan yang lebih terkendali dan konsisten.
    #     </div>
    #     """, unsafe_allow_html=True)
    ...
    #     <div class='section-header'>
    #         <span class='icon'>📉</span>
    #         <span class='title'>Beban Pengeluaran Status Low</span>
    #     </div>
    #     <div class='section-desc'>Kategori pengeluaran yang paling besar proporsinya pada cluster Low.</div>
    #     """, unsafe_allow_html=True)

    #     low_df      = df[df["Status_Label"] == "Low"]
    #     avg_expense = low_df[ALL_RATIO_COLS].mean().sort_values(ascending=False) * 100
    #     nice_labels = [EXPENSE_LABELS.get(c, c) for c in avg_expense.index]

    #     # gradient merah berdasarkan rank
    #     reds = plt.cm.Reds(np.linspace(0.85, 0.35, len(avg_expense)))

    #     fig, ax = plt.subplots(figsize=(9, 5))
    #     fig, ax = setup_ax(fig, ax)

    #     bars = ax.barh(nice_labels[::-1], avg_expense.values[::-1],
    #                    color=reds[::-1], height=0.6, edgecolor="none")

    #     for bar, val in zip(bars, avg_expense.values[::-1]):
    #         ax.text(val + 0.3, bar.get_y() + bar.get_height()/2,
    #                 f"{val:.1f}%", va='center', color='white',
    #                 fontsize=10, fontweight='bold')

    #     ax.set_xlabel("Rata-rata Proporsi (%)")
    #     ax.set_title("Komposisi Pengeluaran — Cluster Low", fontsize=13, fontweight="bold")
    #     plt.tight_layout()
    #     st.pyplot(fig)

    #     top1, top2 = nice_labels[0], nice_labels[1]
    #     st.markdown(f"""
    #     <div class='insight-box'>
    #         Dua pengeluaran terbesar cluster Low adalah <strong>{top1}</strong> dan <strong>{top2}</strong>.
    #         Besarnya alokasi ke kebutuhan pokok mencerminkan tekanan finansial yang tinggi,
    #         sehingga menyisakan sedikit ruang untuk menabung atau berinvestasi.
    #     </div>
    #     """, unsafe_allow_html=True)
    ...
    #     <div class='section-header'>
    #         <span class='icon'>🎓</span>
    #         <span class='title'>Pengaruh Education terhadap Financial Status</span>
    #     </div>
    #     <div class='section-desc'>Apakah proporsi pengeluaran pendidikan berkorelasi dengan status finansial?</div>
    #     """, unsafe_allow_html=True)

    #     edu_mean = df.groupby("Status_Label")["Education_Ratio"].mean() * 100
    ...
    #     colors = [COLOR_MAP[l] for l in labels]
    #     vals   = [edu_mean.get(l, 0) for l in labels]

    #     bars = ax.bar(xs, vals, color=colors, width=0.5, edgecolor="none")
    ...
    #     ax.set_xticks(xs)
    #     ax.set_xticklabels(labels, color="#9aa4b2")
    #     ax.set_ylabel("Education Ratio (%)")
    #     ax.set_title("Rata-rata Education Ratio per Cluster", fontsize=13, fontweight="bold")
    #     plt.tight_layout()
    #     st.pyplot(fig)

    #     st.markdown("""
    #     <div class='insight-box'>
    #         <strong>Temuan:</strong> Education Ratio cenderung meningkat seiring naiknya status finansial.
    #         Ini mengindikasikan bahwa investasi dalam pendidikan dapat berperan sebagai <em>jangka panjang enabler</em>
    #         menuju stabilitas finansial yang lebih baik — meski kausalitasnya perlu dikonfirmasi lebih lanjut.
    #     </div>
    #     """, unsafe_allow_html=True)